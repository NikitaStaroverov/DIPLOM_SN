 ...
 */
export async function fetchYandexWeather(lat: number, lon: number): Promise<WeatherNow> {
  const key = import.meta.env.VITE_YANDEX_WEATHER_KEY as string | undefined;
  if (!key) {
    return { temperature: null, humidity: null, error: 'Не задан VITE_YANDEX_WEATHER_KEY (см. README).' };
  }
  ...
  try{
    ...
    return { temperature: null, humidity: null, error: e?.message ?? 'Ошибка сети' };
  }
}
