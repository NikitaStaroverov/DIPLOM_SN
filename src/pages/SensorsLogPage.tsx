...
function formatLine(ts: number, id: string, wet: number, temp: number, chg: number){
  const d = new Date(ts);
...
export default function SensorsLogPage(){
  ...
  const lines = useMemo(() => {
    ...
    return rs.slice(-3000).map(r => formatLine(r.ts, r.sensorId, r.wetness, r.temperature, r.charge));
  ...
  return (
    <div className="card">
      ...
      <div className="muted" style={{ marginBottom: 12 }}>
        Формат: <span className="kbd">Time_ID_wetness_temp_charge</span>. Новые строки добавляются каждые ~10 секунд (демо).
      </div>
      ...
      <div className="card" style={{ padding: 12, background: 'rgba(0,0,0,.18)' }}>
        <pre style={{ margin: 0, maxHeight: 520, overflow: 'auto', whiteSpace: 'pre-wrap' }}>
