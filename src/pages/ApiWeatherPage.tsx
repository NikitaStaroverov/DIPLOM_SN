...
export default function ApiWeatherPage(){
  ...
  useEffect(() => { load(); }, [fieldId]); // eslint-disable-line

  return (
    <div className="card">
      <div className="h1">API (Яндекс.Погода)</div>
      <div className="muted" style={{ marginBottom: 12 }}>
        Текущие значения температуры/влажности по координатам поля. Для работы нужен ключ и (скорее всего) прокси из-за CORS.
      </div>
      ...
      <div className="grid2">
        <div className="card">
          ...
          <div className="muted">Координаты: {field?.coords.lat}, {field?.coords.lon}</div>
          ...
          {data?.error ? (
            <div style={{ color: 'var(--warn)' }}>
              ...
              <div className="muted" style={{ marginTop: 6 }}>
                Подсказка: добавьте <span className="kbd">VITE_YANDEX_WEATHER_KEY</span> и настройте прокси (см. README).
              </div>
            </div>
          ) : (
            <>
              <div className="row" style={{ marginTop: 8 }}>
                <span className="badge">Температура: <b>{data?.temperature ?? '—'}</b>°C</span>
                <span className="badge">Влажность: <b>{data?.humidity ?? '—'}</b>%</span>
              </div>
            </>
          )}
        </div>
