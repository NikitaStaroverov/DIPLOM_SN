...
export default function MapPage(){
  ...
  const initialFieldId = (location.state as any)?.fieldId ?? (fields[0]?.id ?? '1');
  ...
  const layout = useMemo(() => makeLayout(field?.sensors ?? []), [field?.sensors]);

  const sensorStatus = (id: string): Status => {
    const r = latestBySensor.get(id);
    if (!r) return 'warn';
    const wet = metricStatus(r.wetness, thresholds.wetness.warnMin, thresholds.wetness.warnMax, thresholds.wetness.dangerMin, 'min');
    const temp = metricStatus(r.temperature, thresholds.temperature.warnMin, thresholds.temperature.warnMax, thresholds.temperature.dangerMax, 'max');
    const chg = metricStatus(r.charge, thresholds.charge.warnMin, thresholds.charge.warnMax, thresholds.charge.dangerMin, 'min');
    ...
    <div className="card">
      ...
      <div className="card" style={{ padding: 12 }}>
        <svg width="100%" viewBox="0 0 820 420" style={{ display: 'block' }}>
          <rect x="10" y="10" width="800" height="400" rx="16" fill="rgba(255,255,255,.03)" stroke="rgba(255,255,255,.08)"/>
          <text x="30" y="42" fill="rgba(123, 142, 209, 0.85)" fontSize="16">{field.name}</text>
          ...
          {layout.map((s, idx) => {
            ...
            return (
              <g key={s.id}>
                ...
                {idx > 0 && <line x1={layout[idx-1].x} y1={layout[idx-1].y} x2={s.x} y2={s.y} stroke="rgba(255,255,255,.08)" />}
                <circle cx={s.x} cy={s.y} r="26" fill="rgba(0,0,0,.15)" stroke="rgba(255,255,255,.10)" />
                <circle cx={s.x} cy={s.y} r="18" fill={c} opacity="0.85" />
                <text x={s.x} y={s.y+5} textAnchor="middle" fill="#0b1020" fontWeight="700" fontSize="12">{s.id}</text>

                {r && (
                  <text x={s.x} y={s.y+42} textAnchor="middle" fill="rgba(106, 122, 180, 0.75)" fontSize="11">
